...
def intersection_approx(w, u):
    (A,B), (C,D) = w, u

    det = lambda A, B: A[0]*B[1] - A[1]*B[0]
    dist = lambda A, B: ((A.x-B.x)**2 + (A.y-B.y)**2)**0.5
    equal = lambda a, b: (b!=0 and abs(1-a/b) < 1e-5) or a==b
    on_segment = lambda p, A, B: equal(dist(A,p)+dist(p,B), dist(A,B))
#     on_segment = lambda p, A, B: abs(det((p.x-A.x, p.y-A.y), (p.x-B.x, p.y-B.y))) < 1e-6 and (A.x<=p.x<=B.x or A.x>=p.x>=B.x) and (A.y<=p.y<=B.y or A.y>=p.y>=B.y) 
