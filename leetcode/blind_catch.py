...
moves = list(range(8))
# seqs = itertools.chain(*(itertools.permutations(moves, i) for i in range(1,len(moves)+1)))
seq_gens = [None] + [itertools.permutations(moves, i) for i in range(1,len(moves)+1)]
gens_left = list(range(len(seq_gens)-1,0,-1))
...
try:
    while gens_left:
        ...
        to_remove = []
        for gi in gens_left:
            if (len(guesses)+1) % gi == 0 or gi == gens_left[-1]:
                try:
                    while True:
                        seq = next(seq_gens[gi])
                        loc = [0]*len(moves)
...
#         guesses.append(loc)
#         tried.append(seq)
        print(len(guesses), cnt, loc, seq, gens_left)
#         1/0
except StopIteration:
    print("Finished")
